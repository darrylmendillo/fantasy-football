...
| ~~Per-user cache keying in `client.py`~~ **WITHDRAWN 2026-08-17** | Claimed Phase 1 caches were per-process and leaked across users. **This was wrong** — they are per-instance (`client.py:175,178`), and per-request construction already isolates users. Disproved by writing T008's test and watching it pass against unmodified code | N/A — no fix was needed. The tests are retained as a regression guard for if a data source is ever pooled or memoised. The real open risk is the inverse: per-request construction re-seeds the player universe every call, a rate-limit exposure (spec 001 R5) |
